...
const postTax = prices.map(price => price + (price * taxRate))
...
const sum = populations.reduce((total, num) => total + num)
...
const totals = orders.map(item => (item.price + (item.price * item.tax)))
